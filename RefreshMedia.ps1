...
. "$PSScriptRoot\includeUtilities.ps1" | Out-Null
. "$PSScriptRoot\DU.ps1" | Out-Null
. "$PSScriptRoot\FoD.ps1" | Out-Null
. "$PSScriptRoot\LangPack.ps1" | Out-Null
...
function CheckFreeSpace {
    ...
    try {
        [int]$mediaSizeMB = "{0:N0}" -f ((Get-ChildItem $oldMediaPath -Recurse | Measure-Object -Property Length -Sum -ErrorAction Stop).Sum / 1MB)
